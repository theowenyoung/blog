...
**API note:** Use `bodyMarkdown` (not `body` TipTap JSON) for post content. Server handles markdown conversion.
...
**Custom URL path:** `blog/{original-path}` e.g. `blog/actionsflow`
**State file:** `temp-state.json` (gitignored via `temp*` rule)
**API base:** `https://jant.localtest.me` (override with JANT_BASE_URL env)
